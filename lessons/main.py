...
# Base.metadata.create_all(bind=engine) # create_all is syncronus
@asynccontextmanager # this is for asyncronous
...
async def home(request: Request, db: Annotated[AsyncSession, Depends(get_db)]):
    # result = await db.execute(select(models.Post)) # for sync
...
async def user_posts_page(request: Request, user_id: int, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    return templates.TemplateResponse(
        request,
        "user_posts.html",
        {"user": user, "posts": user_posts, "title": f"{user.username}'s Posts"},
    )
...
async def update_user(user_id: int, user_update: UserUpdate, db: Annotated[AsyncSession, Depends(get_db)]):
    ...
    if user_update.username is not None and user_update.username != user.name:
...
@app.delete("/api/posts/{user_id}", status_code=status.HTTP_204_NO_CONTENT)
...
async def update_complete_post(post_id: int, post_data: PostCreate, db: Annotated[AsyncSession, Depends(get_db)]):
